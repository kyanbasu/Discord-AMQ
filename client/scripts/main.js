...
const socket = io(window.location.href.split('/').slice(0,3).join('/').replace("https", "wss"), {
...
document.getElementById("playerTypeSwitch").addEventListener('change', () => {
  if(document.getElementById("playerTypeSwitch").checked) selectedPlayerType = "webm"
...
socket.on('correctlyGuessed', e => {
  displayMessage(`<span style="color: var(--maincontrast)">Poprawnie zgadli: ${e}</span>`)
})
...
function sendMessage(){
  ...
  if (text.length > 0 && auth)
      socket.emit('message', `${discordSdk.guildId}/${discordSdk.channelId}`, `${auth.user.global_name}: ${text}`)
...
import { appendUserAvatar, appendVoiceChannelName, removeFadeOut, displayMessage } from "./helpers";
...
document.getElementById('volume-slider').style.background = 'linear-gradient(0deg, rgb(80,0,20), rgb(80,0,20) ' + 
  Math.floor(document.getElementById('volume-slider').value*100-1) + '%, #363232 ' + Math.floor(document.getElementById('volume-slider').value*100) + '%, #363232)'

document.getElementById('volume-slider').oninput = () => {
  var value = document.getElementById('volume-slider').value
  videoPlayer.volume = value
  document.getElementById('volume-slider').style.background = 'linear-gradient(0deg, rgb(80,0,20), rgb(80,0,20) ' + Math.floor(value*100-1) + '%, #363232 ' + Math.floor(value*100) + '%, #363232)'
}

videoPlayer.ontimeupdate = () => {
  if(videoPlayer.currentTime < options.guessTime)
    document.getElementById('progress').style.width = `${(videoPlayer.currentTime / options.guessTime)*100}%`
  else
    document.getElementById('progress').style.width = `${(videoPlayer.currentTime / videoPlayer.duration)*100}%`

  if(videoPlayer.duration - videoPlayer.currentTime < 5 && !videoPlayer.triggeredSkip){
    videoPlayer.triggeredSkip = true
    Skip();
  }
}

videoPlayer.onended = () => { player.hidden = true }
...
document.getElementById('animelistname').addEventListener('change', () => {
  ...
  pendingALUpdate = setTimeout(() => {
    socket.emit('updateAL', `${discordSdk.guildId}/${discordSdk.channelId}`, auth.user, document.getElementById('animelistname').value)
  }, 3000);
})
...
function updateOptions(options){
  ...
  pendingOptionsUpdate = setTimeout(() => {
    socket.emit('updateOptions', `${discordSdk.guildId}/${discordSdk.channelId}`, auth.user, options)
  }, 1000);
}
...
function setupOptionsGUI(){
  ...
  queueSizeInput.addEventListener('change', () => {
    queueSizeInput.value = Math.min(queueSizeInput.max, Math.max(queueSizeInput.min, queueSizeInput.value))
  ...
  guessTimeInput.addEventListener('change', () => {
    guessTimeInput.value = Math.min(guessTimeInput.max, Math.max(guessTimeInput.min, guessTimeInput.value))
  ...
  guessesCount.addEventListener('change', () => {
    guessesCount.value = Math.min(guessesCount.max, Math.max(guessesCount.min, guessesCount.value))
...
function Guess(evt){
  socket.emit('guess', auth.user, `${discordSdk.guildId}/${discordSdk.channelId}`, evt.currentTarget.index)
  for (let i = 0; i < options.guessesCount; i++) {
    document.getElementById(`guess${i}`).classList.remove('guessButton');
  }
  document.getElementById(`guess${evt.currentTarget.index}`).classList.add('guessButton')
}
